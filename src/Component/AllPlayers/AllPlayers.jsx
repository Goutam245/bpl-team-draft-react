        ...
        <div>
            <div className="flex justify-between">
                ...
                <div className="flex flex-col sm:flex-row gap-2 z-10">
                    <button onClick={()=>handleAvailableBtn('available')} className={`${available ? "btn btn-warning" : "btn"}`}>Available</button>
                    <button onClick={()=>handleAvailableBtn('selected')} className={`${available ? "btn" : "btn btn-warning"} w-32 md:w-auto`}>Selected<span>({selectedPlayer.length})</span></button>
                </div>
            </div>
